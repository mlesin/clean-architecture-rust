...
async fn import_dog_facts_fixtures(conn: &DbConnection) {
    ...
    let conn = conn.get_pool().await.expect("couldn't get db connection from pool");
    let mut query_builder: QueryBuilder<Postgres> = QueryBuilder::new("INSERT INTO dog_facts(id, fact) ");
