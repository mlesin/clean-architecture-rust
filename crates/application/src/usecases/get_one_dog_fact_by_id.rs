...
use crate::{repositories::dog_facts::DogFactsRepository, usecases::interfaces::UseCase, utils::error_handling_utils::ErrorHandlingUtils};
use domain::{dog_fact_entity::DogFactEntity, error::ApiError};

pub struct GetOneDogFactByIdUseCase<'a> {
    dog_fact_id: &'a i32,
    repository: &'a dyn DogFactsRepository,
}

impl<'a> GetOneDogFactByIdUseCase<'a> {
    pub fn new(dog_fact_id: &'a i32, repository: &'a dyn DogFactsRepository) -> Self {
...
mod tests {
    ...
    use crate::repositories::dog_facts::MockDogFactsRepository;
    ...
    async fn test_should_return_error_with_generic_message_when_unexpected_repo_error() {
        ...
        let mut dog_fact_repository = MockDogFactsRepository::new();
        dog_fact_repository
